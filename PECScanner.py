...
tool owned by the Private Eyes.
___________________________________________
...
def icmp_ping(host):
    ...
    ans.summary(lambda (s,r): r.sprint("%IP.src% is alive oof zombie -.-"))

def tcp_ping(host,dport):
    ans, unans = sr(IP(dst=host)/TCP(dport,flags="S"))
    ans.summary(lambda (s,r): r.sprintf("%IP.src% is alive oof zombie -.-"))
...
levels with pcap file:
  ...
  icmp : extract icmp headers
	    ...
	    parser.add_argument("-s","--serveur", dest="serveur",help="attack server IP like the movies :3")
	    ...
	    eport = args.eport	
			...
			if file and level == "eth":
            	...
            	print '''usage: PECScanner.py [-h] [-s SERVEUR] [-p LEVEL] [-d SPORT] [-t EPORT]
                    [-f FILE]
use PECScanner.py -h to help '''	
